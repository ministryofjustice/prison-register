...
class CreateContactDetailsResourceTest : ContactDetailsBaseIntegrationTest() {
  ...
  inner class SocialVisitContacts {
    ...
    fun `When contact details are created with maintain ref date role , isCreated is return and persisted`() {
      ...
      // When
      val responseSpec =
        doCreateContactDetailsAction(endPoint, prisonId, dto, headers = createMaintainRefRoleWithWriteScope())
    ...
    fun `When contact details are created with maintain prison data role, isCreated is return and persisted`() {
      ...
      // When
      val responseSpec =
        doCreateContactDetailsAction(endPoint, prisonId, dto, headers = createMaintainPrisonRoleWithWriteScope())
    ...
    fun `When more than one contact details are created for more than one prison, only one of phone, email, web address is persisted`() {
      ...
      // When
      val responseSpec1 =
        doCreateContactDetailsAction(endPoint1, prisonId1, dto1, headers = createMaintainRefRoleWithWriteScope())
      val responseSpec2 =
        doCreateContactDetailsAction(endPoint1, prisonId1, dto2, headers = createMaintainPrisonRoleWithWriteScope())
      val responseSpec3 =
        doCreateContactDetailsAction(endPoint2, prisonId2, dto1, headers = createMaintainRefRoleWithWriteScope())
      val responseSpec4 =
        doCreateContactDetailsAction(endPoint2, prisonId2, dto2, headers = createMaintainPrisonRoleWithWriteScope())
    ...
    fun `When incorrect formats are used for contact details, then appropriate error messages are shown`() {
      ...
      val dto = ContactDetailsDto(
        ...
        phoneNumber = "I am bit a phone number",
  ...
  inner class OfficialVisitContacts {
    ...
    @Test
    fun `When official visits contact details are created with maintain prison data role, isCreated is return and persisted`() {
      val dto = ContactDetailsDto(departmentType, emailAddress, phoneNumber, webAddress)
    ...
    @Test
    fun `When official visit contact details already exist, then bad request is returned`() {
      ...
      responseSpec.expectStatus().isBadRequest
      assertDeveloperMessage(responseSpec, "Contact details already exist for BRI / official visit department.")
    }
  }
}
