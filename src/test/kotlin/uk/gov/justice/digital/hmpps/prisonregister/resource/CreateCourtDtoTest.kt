...
class CreateCourtDtoTest {

  private lateinit var validator: Validator

  private val valid = CreateCourtDto(
    courtId = "NEWCRT",
    courtName = "New Court",
  ...
  private fun messagesFor(dto: CreateCourtDto): Set<String> = validator.validate(dto).map(ConstraintViolation<CreateCourtDto>::getMessage).toSet()
  ...
  inner class CourtId {
    @Test
    fun `blank court id fails validation`() {
      assertThat(messagesFor(valid.copy(courtId = ""))).contains("Court id is required")
    }

    @Test
    fun `court id shorter than 2 characters fails validation`() {
      assertThat(messagesFor(valid.copy(courtId = "A"))).contains("Court Id must be between 2 and 6 letters")
    }

    @Test
    fun `court id longer than 6 characters fails validation`() {
      assertThat(messagesFor(valid.copy(courtId = "TOOLONG"))).contains("Court Id must be between 2 and 6 letters")
    }
  }

  @Nested
  inner class CourtName {
    @Test
    fun `blank court name fails validation`() {
      assertThat(messagesFor(valid.copy(courtName = ""))).contains("Court name is required")
    }

    @Test
    fun `court name longer than 40 characters fails validation`() {
      assertThat(messagesFor(valid.copy(courtName = "a".repeat(41))))
        .contains("Court name must be no more than 40 characters")
    }
  }
  ...
  @Nested
  inner class CourtTypeCode {
    @Test
    fun `blank court type code fails validation`() {
      assertThat(messagesFor(valid.copy(courtTypeCode = ""))).contains("Court type code is required")
    }

    @Test
    fun `court type code longer than 12 characters fails validation`() {
      assertThat(messagesFor(valid.copy(courtTypeCode = "a".repeat(13))))
        .contains("Court type code must be no more than 12 characters")
    }
  }
