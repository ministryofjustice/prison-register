...
class UpdateContactDetailsResourceTest : ContactDetailsBaseIntegrationTest() {
  ...
  inner class SocialVisitContacts {
    ...
    fun `When contact details are updated with maintain ref data role, then http 200 is returned with persisted updated contact details`() {
      ...
      val updateRequest = ContactDetailsDto(
        departmentType,
        emailAddress = "tom@moj.gov.uk",
        phoneNumber = "01234567880",
        webAddress = "https://mojdigital.blog.gov.uk",
      )

      // When
      val responseSpec =
        doUpdateContactDetailsAction(endPoint, updateRequest, headers = createMaintainRefRoleWithWriteScope())
    ...
    fun `When contact details are updated with maintain prison data role, then http 200 is returned with persisted updated contact details`() {
      ...
      // When
      val responseSpec =
        doUpdateContactDetailsAction(endPoint, updateRequest, headers = createMaintainPrisonRoleWithWriteScope())
    ...
    fun `When more than one contact details are updated for more than one prison, only one of phone, email, web address is persisted`() {
      ...
      val updateRequest1 = ContactDetailsDto(
        SOCIAL_VISIT,
        emailAddress = "tom@moj.gov.uk",
        phoneNumber = "01234567890",
        webAddress = "www.mojdigital.blog.gov.uk",
      )
      val updateRequest2 = ContactDetailsDto(
      ...
      // When
      val responseSpec1 =
        doUpdateContactDetailsAction(endPoint1, updateRequest1, headers = createMaintainRefRoleWithWriteScope())
      val responseSpec2 =
        doUpdateContactDetailsAction(endPoint2, updateRequest2, headers = createMaintainPrisonRoleWithWriteScope())
    ...
    fun `When only phone number is updated and removeIfNull then remove others but keep for others contact details`() {
      ...
      // When
      val responseSpec =
        doUpdateContactDetailsAction(endPoint1, updateRequest, headers = createMaintainRefRoleWithWriteScope())
    ...
    fun `When only contact details are orphaned and false, orphaned details are removed`() {
      ...
      val originalData = createDBData(
        prisonId,
        SOCIAL_VISIT,
        emailAddress = "delete-me@moj.gov.uk",
        phoneNumber = "01234567890",
        webAddress = "www.deleteMe.gov.uk",
      )
      val updateRequest = ContactDetailsDto(
        SOCIAL_VISIT,
        emailAddress = "aled@moj.gov.uk",
        phoneNumber = "01234567880",
        webAddress = "www.aled.gov.uk",
      )
    ...
    fun `When incorrect formats are used for contact details, then appropriate error messages are shown`() {
      ...
      val updateRequest = ContactDetailsDto(
        ...
        phoneNumber = "I an bit a phone number",
        webAddress = "I am not a web address",
      )
    ...
    fun `When contact details are sent with a non existent prison, then bad request is returned`() {
      ...
      assertDeveloperMessage(
        responseSpec,
        "Unable to find uk.gov.justice.digital.hmpps.prisonregister.model.Prison with id AWE",
      )
    }
  }
  ...
  inner class OfficialVisitContacts {
    ...
    @Test
    fun `When official visit contact details are updated with maintain ref data role, then details are persisted`() {
      createDBData(prisonId, departmentType, phoneNumber, emailAddress, webAddress)

      val updateRequest = ContactDetailsDto(
        type = departmentType,
        emailAddress = "legal-updated@bri.justice.gov.uk",
        phoneNumber = "01234567880",
        webAddress = "https://visits.justice.gov.uk/updated",
      )
    ...
    @Test
    fun `When official visit contact details are updated with maintain prison data role, then details are persisted`() {
      createDBData(prisonId, departmentType, phoneNumber, emailAddress, webAddress)

      val updateRequest = ContactDetailsDto(
        departmentType,
        emailAddress = "legal-visits-updated@bri.justice.gov.uk",
        phoneNumber = "01234567880",
        webAddress = "https://visits.justice.gov.uk/official-updated",
      )
    ...
    @Test
    fun `When more than one contact detail is updated for different departments a single instance of phone, email and web address is persisted`() {
      val prisonId2 = "CFI"
      val endPoint2 = getContactDetailsEndPoint(prisonId2)

      // Same details for each contact type
      val updateRequest1 = ContactDetailsDto(departmentType, emailAddress, phoneNumber, webAddress)
      val updateRequest2 = ContactDetailsDto(OFFENDER_MANAGEMENT_UNIT, emailAddress, phoneNumber, webAddress)

      // Persist these in the DB
      createDBData(prisonId, updateRequest1)
      createDBData(prisonId2, updateRequest2)

      val responseSpec1 = doUpdateContactDetailsAction(endPoint, updateRequest1, headers = createMaintainRefRoleWithWriteScope())
      val responseSpec2 = doUpdateContactDetailsAction(endPoint2, updateRequest2, headers = createMaintainPrisonRoleWithWriteScope())

      responseSpec1.expectStatus().isOk
      responseSpec2.expectStatus().isOk

      assertThat(testPhoneNumberRepository.getPhoneNumberCount(updateRequest1.phoneNumber!!)).isEqualTo(1)
      assertThat(testEmailAddressRepository.getEmailCount(updateRequest1.emailAddress!!)).isEqualTo(1)
      assertThat(testWebAddressRepository.getWebAddressCount(updateRequest1.webAddress!!)).isEqualTo(1)
    }

    @Test
    fun `When original official visit contact details are orphaned, and removeIfNull is false, the original details are removed`() {
      val endPoint1 = getContactDetailsEndPoint(prisonId, removeIfNull = false)

      val originalData = createDBData(prisonId, departmentType, phoneNumber, emailAddress, webAddress)
      ...
      assertThat(testPhoneNumberRepository.getPhoneNumberCount(originalData.phoneNumber!!)).isEqualTo(0)
      assertThat(testEmailAddressRepository.getEmailCount(originalData.emailAddress!!)).isEqualTo(0)
      assertThat(testWebAddressRepository.getWebAddressCount(originalData.webAddress!!)).isEqualTo(0)
    }

    @Test
    fun `When official visit contact details are updated with removeIfNull false, then remove the orphaned details but retain where another department uses`() {
      val endPoint1 = getContactDetailsEndPoint(prisonId, removeIfNull = false)

      // Create full contact details for an official visit
      val persisted = createDBData(prisonId, departmentType, phoneNumber, emailAddress, webAddress)

      // Create a second contact for videolink - with same email and phone number, but different web address
      createDBData(
        prisonId,
        departmentType = VIDEOLINK_CONFERENCING_CENTRE,
        phoneNumber,
        emailAddress,
        webAddress = "https://visits.justice.gov.uk/new-address",
      )

      // Update the official visit contact to remove its phone number and email address - but retain its web address
      val updateRequest = ContactDetailsDto(departmentType, webAddress = webAddress)

      val responseSpec = doUpdateContactDetailsAction(endPoint1, updateRequest, headers = createMaintainRefRoleWithWriteScope())

      val contactDetailsDto = getContactDetailsDtoResults(responseSpec.expectBody())

      assertThat(persisted.type).isEqualTo(contactDetailsDto.type)
      assertThat(persisted.emailAddress).isEqualTo(contactDetailsDto.emailAddress)
      assertThat(updateRequest.webAddress).isEqualTo(contactDetailsDto.webAddress)
      assertThat(persisted.phoneNumber).isEqualTo(contactDetailsDto.phoneNumber)
    }

    @Test
    fun `When incorrect formats are used for official visit contact details, then appropriate error messages are shown`() {
      val updateRequest = ContactDetailsDto(
        departmentType,
        emailAddress = "I am not an email",
        phoneNumber = "I am not a phone number",
        webAddress = "I am not a web address",
      )
